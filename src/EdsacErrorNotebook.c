...
static void append_linky_text_buffer(LinkyBuffer *linky_buffer, unsigned int rack_no, unsigned int chassis_no, unsigned int valve_no, const char* msg);
...
static void append_linky_text_buffer(LinkyBuffer *linky_buffer, unsigned int rack_no, unsigned int chassis_no, unsigned int valve_no, const char* msg) {
    ...
    const gsize valve_end = offset + message->len - 2; // colon, space
    ...
    Clickable *valve_data = malloc(sizeof(Clickable));
...
static void clicked(__attribute__((unused)) const GtkTextTag *tag, const GtkTextView *parent, const GdkEvent *event,
    ...
    if (event->type == GDK_BUTTON_RELEASE && event_btn->button == 1) {
        ...
        if (NULL != found) {
            ...
            gtk_notebook_set_current_page(GTK_NOTEBOOK(notebook), tab->page_id);
            return;
        }
...
static void edsac_error_notebook_instance_init(EdsacErrorNotebook *self) {
    ...
    // dummy data
    append_linky_text_buffer(all, 1, 2, 3, "four");
